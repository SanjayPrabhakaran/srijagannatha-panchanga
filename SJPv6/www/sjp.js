...
//below function to get vikrama samvatsara from Kaliyear
Date.prototype.getVikramaSamvatsara = function () {
    k = this.getKaliAbda();
    //Now calculate vikram samvatsara from k the kali year
    vikramaSamvatsara = k - 3101 + 57;
    return vikramaSamvatsara;
}
...
function initSJP() {
    ...
    if (params[1] !== null && params[1] !== undefined) {
        ...
        for (i = 0; i < params.length; ++i) {
            ...
            p[1] = p[1].replace(/\+/g, " ");
            params[p[0]] = unescape(p[1]);//replace this line with valid unescape as unescape is deprecated
            params[p[0]] = decodeURIComponent(p[1]);

            if (p[0] === "submit" && p[1] === "Calculate") submit = true;
        }
    }
    window.status = "Set values."; //Window status is not supported in chrome so replace with generic message
